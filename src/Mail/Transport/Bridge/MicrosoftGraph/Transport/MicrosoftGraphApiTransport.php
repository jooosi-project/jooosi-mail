...
class MicrosoftGraphApiTransport extends AbstractApiTransport
{
    ...
    {
        $message = ['sender' => $this->getEmailAddress($envelope->getSender()), 'subject' => $email->getSubject(), 'body' => $this->getBodyPayload($email), 'importance' => $this->getImportanceLevel($email), 'toRecipients' => array_map($this->getEmailAddress(...), $this->getRecipients($email, $envelope))];
        if ($email->getFrom()) {
