...
final class SweegoTransportFactory extends AbstractTransportFactory
{
    public function create(Dsn $dsn): TransportInterface
    {
        return match ($dsn->getScheme()) {
            'sweego', 'sweego+smtp' => new \JooosiMail\Mail\Transport\Bridge\Sweego\Transport\SweegoSmtpTransport($dsn->getHost(), $dsn->getPort(), $this->getUser($dsn), $this->getPassword($dsn), $this->dispatcher, $this->logger),
            'sweego+api' => (new \JooosiMail\Mail\Transport\Bridge\Sweego\Transport\SweegoApiTransport($this->getUser($dsn), $this->client, $this->dispatcher, $this->logger))->setHost('default' === $dsn->getHost() ? null : $dsn->getHost())->setPort($dsn->getPort()),
