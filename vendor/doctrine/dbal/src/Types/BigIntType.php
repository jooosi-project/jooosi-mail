...
class BigIntType extends Type implements PhpIntegerMappingType
{
    ...
    public function convertToPHPValue(mixed $value, AbstractPlatform $platform): int|string|null
    {
        ...
        assert(is_string($value), 'DBAL assumes values outside of the integer range to be returned as string by the database driver.');
        if ($value > PHP_INT_MIN && $value < PHP_INT_MAX || $value === (string) (int) $value) {
            return (int) $value;
        }
        return $value;
    }
